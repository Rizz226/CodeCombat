# All the enemy types I have come across

enemies = self.findEnemies()
enemy = self.findNearest(enemies)
fangriders = self.findByType("fangrider")
fangrider = self.findNearest(fangriders)
throwers = self.findByType("thrower")
thrower = self.findNearest(throwers)
shamans = self.findByType("shaman")
shaman = self.findNearest(shamans)   
ogres = self.findByType("ogre")
ogre = self.findNearest(ogres)
munchkins = self.findByType("munchkin")
munchkin = self.findNearest(munchkins)
scouts = self.findByType("scout")
scout = self.findNearest(scouts)
yaks = hero.findByType("yak")
yak = hero.findNearest(yaks)
sYaks = hero.findByType("sand-yak")
sYak = hero.findNearest(sYaks)
# Fences and many doors
other = undefined == 0 == null
